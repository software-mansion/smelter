...
export class SmelterManager {
  ...
  public async init() {
    ...
    await SmelterInstance['instance'].registerShader('example_shader', {
      source: EXAMPLE_SHADER,
    });
    for (const shader of shadersController.shaders) {
      await registerShaderFromFile(SmelterInstance['instance'], shader.id, path.join(__dirname, `../shaders/${shader.shaderFile}`));
    }
