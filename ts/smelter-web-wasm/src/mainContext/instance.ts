  ...
  private async handleRequest(request: ApiRequest): Promise<any> {
    ...
    if (route.type == 'input') {
      if (route.operation === 'register') {
        ...
        await this.instance.handleUnregisterInput(route.id);
      }
      ...
      if (route.operation === 'register') {
        ...
        await this.instance.handleUnregisterOutput(route.id);
      } else if (route.operation === 'update') {
        await this.instance.handleUpdateOutput(route.id, request.body as Api.UpdateOutputRequest);
      }
