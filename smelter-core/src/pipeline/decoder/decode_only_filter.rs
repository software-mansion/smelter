use crate::{Timestamp, pipeline::decoder::EncodedInputEvent};
...
pub(crate) struct DecodeOnlyFilter {
    ...
    ranges: Vec<(Timestamp, Option<Timestamp>)>,
...
impl DecodeOnlyFilter {
            ...
            EncodedInputEvent::Chunk(chunk) => {
                ...
                match (chunk.decode_only, self.ranges.last_mut()) {
                    ...
                    (true, Some((start, _end))) => {
                        *start = Timestamp::min(*start, chunk.pts);
                    }

                    // modify existing range even if closed if current PTS is smaller
                    (false, Some((_, end))) => {
                        *end = Some(Timestamp::min(end.unwrap_or(Timestamp::MAX), chunk.pts));
                    }
    ...
    /// it was returned.
    pub fn should_drop(&mut self, pts: Timestamp) -> bool {
