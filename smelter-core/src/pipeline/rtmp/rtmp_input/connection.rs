...
use crate::{
    ...
    queue::{QueueTrackOptions, WeakQueueInput},
    utils::InitializableThread,
};

use crate::prelude::*;

const RTMP_BUFFER: Duration = Duration::from_secs(1);
...
pub(crate) fn start_connection_thread(
    ...
    let Some(queue_input) = input.queue_input.upgrade() else {
        return None;
    };
    let (video_sender, audio_sender) = queue_input.queue_new_track(QueueTrackOptions {
        video: true,
        audio: true,
        offset: Some(ctx.queue_ctx.effective_last_pts() + RTMP_BUFFER),
    });
...
impl RtmpConnectionState {
    ...
    fn normalize_pts(&mut self, pts: Duration) -> Duration {
        let offset = *self.first_pts.get_or_insert(pts);
        pts.saturating_sub(offset)
    }
}
