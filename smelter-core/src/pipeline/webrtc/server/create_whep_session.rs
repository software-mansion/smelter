...
pub async fn handle_create_whep_session(
    ...
    let stats_sender = WhepOutputStatsSender::new(output_ref.clone(), ctx.stats_sender.clone());
    let cleanup_session_handler = OnCleanupSessionHdlr::new(&outputs, &output_ref, &session_id);
    peer_connection.on_peer_connection_cleanup(cleanup_session_handler, stats_sender);
