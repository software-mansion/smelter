...
/// How long the accept epoll waits between shutdown checks.
...
fn bind_listener(port: u16) -> Result<SrtSocket, SrtOutputError> {
    let socket = SrtSocket::new()?;
    socket.set_nonblocking(true)?;
    let addr = SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::UNSPECIFIED, port));
    socket
        .bind(addr)
        .map_err(|e| SrtOutputError::Bind(port, e))?;
    socket.listen(1)?;
    Ok(socket)
}
...
impl SrtSenderThread {
        ...
        loop {
            ...
            if self.listener.is_none() {
                match bind_listener(self.port) {
                    ...
                    Err(err) => {
                        warn!("Failed to re-bind SRT listener: {err}");
                        return;
                    }
                }
            }
            ...
            // Drop the listener so additional callers are refused at the OS
            // level until the current caller disconnects.
            self.listener = None;

            // The listener was non-blocking so the accepted socket inherits
            // that. Flip it back to blocking — we rely on blocking sends to
            // provide natural back-pressure and to surface peer disconnects
            // as send errors.
    ...
    fn wait_for_caller(&self) -> WaitResult {
        let listener = self.listener.as_ref().expect("listener must be bound");
        let epoll = match SrtEpoll::new() {
            Ok(e) => e,
            Err(err) => {
                warn!("Failed to create SRT epoll: {err}");
                return WaitResult::EncoderClosed;
            }
        };
        if let Err(err) = epoll.add(listener, EPOLL_IN | EPOLL_ERR) {
            warn!("Failed to add SRT listener to epoll: {err}");
            return WaitResult::EncoderClosed;
        }
        ...
        loop {
            ...
            match epoll.wait(&mut events, ACCEPT_POLL_TIMEOUT_MS) {
                ...
                Ok(_) => match listener.accept() {
                    Ok((sock, _addr)) => return WaitResult::Caller(sock),
                    Err(err) => {
