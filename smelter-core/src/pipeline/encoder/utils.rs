...
pub(super) fn bitrate_from_resolution_framerate(
    ...
    const BPP: f64 = 0.08;
    let width = NonZero::new(u32::max(resolution.width as u32, 1)).unwrap();
    let height = NonZero::new(u32::max(resolution.height as u32, 1)).unwrap();

    let average_bitrate =
        (width.get() * height.get()) as f64 * (framerate.num as f64 / framerate.den as f64) * BPP;
