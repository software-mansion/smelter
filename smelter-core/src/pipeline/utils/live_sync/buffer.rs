...
impl BufferingStrategy {
    ...
    /// Whether the buffer `anchor` produces left the range this strategy
    /// allows, so it has to be re-anchored at [`desired_anchor`](Self::desired_anchor).
    pub(super) fn needs_reanchor(
        &self,
        estimation: EdgeEstimate,
