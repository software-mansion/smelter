...
///
/// Required features:
/// - Buffer up to reaching edge:
///   - If less than desired buffer then use what you have.
///   - If more than desired buffer then cut off to have at least desired buffer (cut on keyframes).
/// - Packet order is not guaranteed, jitter buffer might be necessary
///   - Acceptable as an option
/// - Configuration needs to allow something like HLS, but also minimal latency e.g. moq
///   - For RTMP I want to say 2 second buffer
///   - For MoQ I want to have option to (2 second) or minimal latency
///     - 2 second option, (desired buffer for initial state or resync, min/max buffer to adjust
///     when packet are too late/to early)
///     - for low latency
///
