...
/// Minimum lead over the playback position a chunk needs to still reach the
/// queue in time.
...
struct SharedAnchor {
    ...
    /// the pts step between consecutive releases sizes the slew of `current`
    /// towards `target`. Reset (together with the entire anchor) on
    /// discontinuity.
