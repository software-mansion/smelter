...
pub(super) fn start_track_decision<B: LiveSyncBuffer>(
    ...
    // At this point the tracks are independent (the merge is left to the corrections).
    // All the early exits below should never happen.
    ...
    Some(Mode::Independent(match kind {
        ...
        //
        // Select audio as an leader. Calculate anchor so it produces the same anchor as
                ...
                && mode.is_started(TrackKind::Audio) =>
        {
            ...
            IndependentMode {
                ...
                leader_anchor: match mode {
                    // when video was starting it might have already polluted shared
                    // audio track, so recalculating target
