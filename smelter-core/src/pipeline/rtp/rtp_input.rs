    ...
    fn start_rtp_demuxer_thread(
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                run_rtp_demuxer_thread(sync_point, jitter_buffer_init, receiver, video, audio)
            })
            .unwrap();
    }
...
fn run_rtp_demuxer_thread(
    ...
    let mut audio = audio_handle.map(|handle| TrackState {
        jitter_buffer: RtpJitterBuffer::new(
            ...
            RtpTimestampSync::new(&sync_point, handle.sample_rate),
        ),
        handle,
        eos_received: false,
    });
    let mut video = video_handle.map(|handle| TrackState {
        jitter_buffer: RtpJitterBuffer::new(
            jitter_buffer_init.clone(),
            RtpTimestampSync::new(&sync_point, 90_000),
        ),
        handle,
        eos_received: false,
    });
