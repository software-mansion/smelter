...
#[derive(Debug)]
pub struct ArgbToRgbaConverter {
    pipeline: wgpu::RenderPipeline,
    sampler: Sampler,
}

impl ArgbToRgbaConverter {
    pub fn new(
        device: &wgpu::Device,
        yuv_textures_bind_group_layout: &wgpu::BindGroupLayout,
        dst_view_format: wgpu::TextureFormat,
    ) -> Self {
        let shader_module = device.create_shader_module(wgpu::include_wgsl!("argb_to_rgba.wgsl"));
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("ARGB to RGBA color converter render pipeline"),
    ...
    pub fn convert(&self, ctx: &WgpuCtx, src_bg: &wgpu::BindGroup, dst_view: &wgpu::TextureView) {
        let mut encoder = ctx
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("ARGB to RGBA color converter encoder"),
            });

        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("ARGB to RGBA color converter render pass"),
