    ...
    fn interpolate(start: &Self, end: &Self, state: InterpolationState) -> Self {
        match (start, end) {
            (
                ...
                Position::Static { width: width_end, height: height_end },
            ) => Self::Static {
...
impl ContinuousValue for BorderRadius {
    fn interpolate(start: &Self, end: &Self, state: InterpolationState) -> Self {
        Self {
            ...
            top_right: ContinuousValue::interpolate(
                &start.top_right,
                &end.top_right,
                state,
            ),
            ...
            bottom_left: ContinuousValue::interpolate(
                &start.bottom_left,
                &end.bottom_left,
                state,
            ),
...
impl ContinuousValue for BoxShadow {
    fn interpolate(start: &Self, end: &Self, state: InterpolationState) -> Self {
        Self {
            ...
            blur_radius: ContinuousValue::interpolate(
                &start.blur_radius,
                &end.blur_radius,
                state,
            ),
