...
use crate::{
    ...
    wgpu::{WgpuCtx, texture::ArgbLinearTexture},
};

use super::convert_linear_to_srgb::RgbToSrgbConverter;

pub(super) struct ArgbInput {
    upload_textures: ArgbLinearTexture,
    argb_bind_group: wgpu::BindGroup,
...
impl ArgbInput {
    ...
    fn maybe_recreate(&mut self, ctx: &WgpuCtx, resolution: Resolution) {
        ...
        self.upload_textures = ArgbLinearTexture::new(ctx, resolution);
        self.argb_bind_group = self.upload_textures.new_bind_group(ctx);
