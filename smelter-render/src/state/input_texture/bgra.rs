...
use crate::{
    ...
    wgpu::{WgpuCtx, texture::BgraLinearTexture},
};

use super::convert_linear_to_srgb::RgbToSrgbConverter;

pub(super) struct BgraInput {
    upload_textures: BgraLinearTexture,
...
impl BgraInput {
    ...
    fn maybe_recreate(&mut self, ctx: &WgpuCtx, resolution: Resolution) {
        ...
        self.upload_textures = BgraLinearTexture::new(ctx, resolution);
        self.bgra_bind_group = self.upload_textures.new_bind_group(ctx);
