...
impl DmaBufFrame {
    ...
    pub(crate) fn descriptor(&self) -> Nv12DmaBufDescriptor {
...
pub(super) fn export_nv12_dmabuf_texture(
    ...
    unsafe {
        ...
        let image_flags =
            vk::ImageCreateFlags::MUTABLE_FORMAT | vk::ImageCreateFlags::EXTENDED_USAGE;
        let modifier = select_nv12_modifier(vulkan, image_usage, image_flags)?;
        let modifiers = [modifier];
        let mut external_info = vk::ExternalMemoryImageCreateInfo::default()
            .handle_types(vk::ExternalMemoryHandleTypeFlags::DMA_BUF_EXT);
        let mut drm_info = vk::ImageDrmFormatModifierListCreateInfoEXT::default()
            .drm_format_modifiers(&modifiers);
        let create_info = nv12_image_create_info(resolution, image_flags, image_usage)
        ...
        let memory_type_index =
            find_memory_type_index(&vulkan.memory_properties, &mem_requirements)?;
        ...
        let memory = vk_device.allocate_memory(&allocate_info, None).map_err(|err| {
            DmaBufError::Vulkan(format!(
        ...
        let fd = match vulkan.external_memory_fd.get_memory_fd(&fd_info) {
            Ok(fd) => Arc::new(OwnedFd::from_raw_fd(fd)),
            Err(err) => {
                return Err(DmaBufError::Vulkan(format!(
                    "failed to export NV12 DMA-BUF fd: {err}"
                )));
            }
        };

        let plane0 =
            vulkan::image_plane_layout(&vk_device, image, vk::ImageAspectFlags::PLANE_0);
        let plane1 =
            vulkan::image_plane_layout(&vk_device, image, vk::ImageAspectFlags::PLANE_1);
        let object_size = mem_requirements.size.try_into().map_err(|_| {
            DmaBufError::InvalidLayout(
                "DMA-BUF allocation is larger than Intel Quick Sync can describe".into(),
            )
        })?;
        let objects: Box<[DmaBufObject]> =
            Box::new([DmaBufObject { fd, size: object_size, modifier }]);
        let layer = exported_nv12_layer(plane0, plane1)?;
        let descriptor = Nv12DmaBufDescriptor::new(resolution, objects, layer)?;
...
fn exported_nv12_layer(
    y: vk::SubresourceLayout,
    uv: vk::SubresourceLayout,
) -> Result<Nv12DmaBufLayer, DmaBufError> {
    Ok(Nv12DmaBufLayer {
        planes: [exported_nv12_plane("Y", y)?, exported_nv12_plane("UV", uv)?],
    })
}

fn exported_nv12_plane(
    name: &'static str,
    layout: vk::SubresourceLayout,
) -> Result<DmaBufPlane, DmaBufError> {
    Ok(DmaBufPlane {
        object_index: 0,
        offset: layout.offset.try_into().map_err(|_| {
            DmaBufError::InvalidLayout(format!("NV12 {name} offset does not fit u32"))
        })?,
        pitch: layout.row_pitch.try_into().map_err(|_| {
            DmaBufError::InvalidLayout(format!("NV12 {name} pitch does not fit u32"))
        })?,
    })
}
...
fn select_nv12_modifier(
    ...
    let required = nv12_format_features();
    let modifiers = &vulkan.nv12_modifiers;
    let is_exportable = |modifier: &vk::DrmFormatModifierProperties2EXT| {
        supports_nv12_modifier(modifier, required)
            && validate_nv12_external_image_support(
                &vulkan.instance,
                ...
                Nv12ModifierSupportQuery {
                    ...
                    usage,
                    flags,
                    format_features: required,
                    external_features: vk::ExternalMemoryFeatureFlags::EXPORTABLE,
                    label: "NV12 DMA-BUF export",
                },
            )
            .is_ok()
    };
    ...
    modifiers
        .iter()
        .find(|modifier| modifier.drm_format_modifier != 0 && is_exportable(modifier))
        .or_else(|| modifiers.iter().find(|modifier| is_exportable(modifier)))
        .copied()
