...
pub(crate) unsafe fn wrap_nv12_image_as_wgpu_texture(
    ...
    let hal_device_guard = unsafe {
        wgpu_device.as_hal::<VkApi>().ok_or_else(|| {
            DmaBufError::UnsupportedDevice("NV12 DMA-BUF requires a Vulkan wgpu device".into())
        })?
    };
    let hal_texture = unsafe {
        let hal_usage =
            wgpu::TextureUses::RESOURCE | wgpu::TextureUses::COPY_DST | wgpu::TextureUses::COPY_SRC;
...
pub(crate) unsafe fn image_plane_layout(
    device: &ash::Device,
    image: vk::Image,
...
pub(crate) unsafe fn image_plane_memory_requirements(
    ...
    let mut plane_info = vk::ImagePlaneMemoryRequirementsInfo::default().plane_aspect(plane_aspect);
