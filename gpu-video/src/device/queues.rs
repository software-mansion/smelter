...
impl Queue {
    pub(crate) fn supports_result_status_queries(&self) -> bool {
        self.query_result_status_properties.query_result_status_support == vk::TRUE
    }
    ...
    pub(crate) fn submit_chain_semaphore<K: TrackerKind>(
        ...
        let semaphore_submit_info =
            tracker.semaphore_tracker.next_submit_info(new_wait_state);
        ...
        if let Some(wait_info) = wait_info.as_ref() {
            submit_info =
                submit_info.wait_semaphore_infos(std::slice::from_ref(wait_info));
        }
...
pub(crate) struct QueueIndex<'a> {
    ...
    pub(crate) query_result_status_properties:
        vk::QueueFamilyQueryResultStatusPropertiesKHR<'a>,
}
...
impl QueueIndices<'_> {
    pub(crate) fn queue_create_infos(&self) -> Vec<QueueCreateInfo> {
        [
            self.h264_decode.as_ref().map(|q| (q.family_index, q.queue_count)),
            self.encode.as_ref().map(|q| (q.family_index, q.queue_count)),
...
impl VideoQueues {
    pub(crate) fn new(queues: Box<[Queue]>) -> Option<Self> {
        ...
        Some(Self { queues, current_queue_idx: AtomicUsize::new(0), family_index })
    }
