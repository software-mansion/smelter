...
use crate::{
    EncodedOutputChunk, VideoBackendError,
    backends::vulkan::{
        VulkanCommonError,
        codec::{
            EncodeCodec,
        ...
        wrappers::{
            ...
            ImageWithView, OpenCommandBuffer, ProfileInfo, QueryPool, ResultQuery, ResultQueryPool,
...
fn new_query_pool<C: EncodeCodec>(
    encoding_device: &EncodingDevice,
    profile: C::Profile,
    profile_info: vk::VideoProfileInfoKHR,
    query_count: u32,
) -> Result<ResultQueryPool<EncodeFeedback>, VulkanEncoderError> {
    let encode_capabilities =
        C::encode_codec_profile_capabilities(&encoding_device.native_encode_capabilities, profile)?;
...
pub(crate) struct EncodeSubmission {
    ...
    _in_flight_resources: InFlightEncodeResources,
}
...
pub(crate) struct FullEncoderParameters<C: EncodeCodec> {
    ...
    pub(crate) max_in_flight_submissions: u32,
}
...
pub(crate) struct VulkanEncoder<'a, C: EncodeCodec> {
    ...
    query_pool: ResultQueryPool<EncodeFeedback>,
    profile: C::Profile,
...
impl<'a, C: EncodeCodec + 'a> VulkanEncoder<'a, C> {
    ...
    pub(crate) fn new(
        ...
        let query_pool = new_query_pool::<C>(
            &encoding_device,
            parameters.profile,
