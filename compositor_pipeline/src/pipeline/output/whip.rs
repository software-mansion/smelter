...
impl WhipSenderTask {
    pub fn spawn(
        ...
        pipeline_ctx: Arc<PipelineCtx>,
        whip_ctx: WhipCtx,
    ) -> Result<(Self, WhipSenderTaskHandle), OutputInitError> {
    ...
    fn wait_with_deadline(
        mut result_receiver: oneshot::Receiver<Result<(), WhipError>>,
    ) -> Result<(), OutputInitError> {
        let start_time = Instant::now();
        while start_time.elapsed() < WHIP_INIT_TIMEOUT {
            thread::sleep(Duration::from_millis(500));
    ...
    async fn new(whip_ctx: WhipCtx) -> Result<Self, WhipError> {
        let client = Arc::new(reqwest::Client::new());
        let (peer_connection, video_transceiver, audio_transceiver) =
            init_peer_connection(&whip_ctx).await?;
        ...
        let whip_session_url = connect(peer_connection.clone(), client.clone(), &whip_ctx).await;
        ...
        if let Some(keyframe_sender) = encoder.keyframe_request_sender() {
            if let Some(video_transceiver) = video_transceiver {
                ...
                handle_keyframe_requests(whip_ctx.clone(), video_sender, keyframe_sender.clone())
                    .await;
            }
            if let Some(audio_transceiver) = audio_transceiver {
                let audio_sender = audio_transceiver.sender().await;
                handle_keyframe_requests(whip_ctx.clone(), audio_sender, keyframe_sender.clone())
                    .await;
            }
        }
    ...
    fn run(self) {
        for chunk in packet_stream {
            ...
            match chunk {
                Payload::Video(video_payload) => match video_track.clone() {
                ...
                Payload::Audio(audio_payload) => match audio_track.clone() {
