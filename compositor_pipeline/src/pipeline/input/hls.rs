...
impl HlsInput {
    ...
    fn run_thread(
        ...
        let mut video_discont = DiscontinuityState::default();
        let mut audio_discont = DiscontinuityState::default();
        audio_discont.is_audio = true;
        loop {
            ...
            if let Some((index, time_base, ref sender)) = video {
                video_discont.update(&packet, time_base);
                let pts = video_discont.recalculate_pts(packet.pts(), time_base);
                let dts = video_discont.recalculate_dts(packet.dts(), time_base);
            ...
            if let Some((index, time_base, ref sender)) = audio {
                audio_discont.update(&packet, time_base);
                let pts = audio_discont.recalculate_pts(packet.pts(), time_base);
                let dts = audio_discont.recalculate_dts(packet.dts(), time_base);
...
// TODO(noituri): Do the same for audio
// TODO(noituri): Stops working after a while
impl DiscontinuityState {
    ...
    fn update(&mut self, packet: &Packet, time_base: ffmpeg_next::Rational) {
        ...
        // if self.is_audio && self.discontinuity {
        //     tracing::error!("Audio DTS: {dts}");
        // }
    ...
    fn recalculate_dts(
        ...
        dts.map(|dts| {
            Duration::from_secs_f64(
                (dts as f64 + self.offset) * time_base.numerator() as f64 / time_base.denominator() as f64,
            )
        })
    }
