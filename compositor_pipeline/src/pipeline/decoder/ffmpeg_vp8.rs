...
impl VideoDecoder for FfmpegVp8Decoder {
    ...
    fn new(ctx: &Arc<PipelineCtx>, options: Self::Options) -> Result<Self, InputInitError> {
        ...
        let decoder = decoder.open_as(Into::<Id>::into(parameters.id()))?;
        Ok(Self {
            decoder,
    ...
    fn decode(&mut self, chunk: EncodedChunk) -> Vec<Frame> {
        if chunk.kind != EncodedChunkKind::Video(VideoCodec::VP8) {
            error!("VP8 decoder received chunk of wrong kind: {:?}", chunk.kind);
            return Vec::new();
        }
...
impl FfmpegVp8Decoder {
    fn read_all_frames(&mut self) -> Vec<EncodedChunk> {
        iter::from_fn(|| {
            match self.decoder.receive_frame(&mut self.av_frame) {
                Ok(_) => {
                    let frame = match frame_from_av(&mut self.av_frame) {
                        Ok(frame) => {
...
enum DecoderChunkConversionError {
    #[error(
        "Cannot send a chunk of kind {0:?} to the decoder. The decoder only handles VP8-encoded video."
    )]
