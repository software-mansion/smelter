...
pub fn start_ffmpeg_decoder_thread(
    ...
    std::thread::Builder::new()
        .name(format!("VP8 decoder {}", input_id.0))
        .spawn(move || {
            let _span =
                span!(Level::INFO, "VP8 decoder", input_id = input_id.to_string()).entered();
            run_decoder_thread(
                parameters,
                init_result_sender,
                chunks_receiver,
    ...
    for chunk in chunks_receiver {
        ...
        while decoder.receive_frame(&mut decoded_frame).is_ok() {
            ...
            if frame_sender.send(PipelineEvent::Data(frame)).is_err() {
                debug!("Failed to send frame from VP8 decoder. Channel closed.");
                return;
            }
        }
    }
