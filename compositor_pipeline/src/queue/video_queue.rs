...
use crate::{
    ...
    queue::{
        utils::{EmitEventOnce, OffsetHandler},
        QueueVideoOutput,
    },
};
...
impl VideoQueue {
    ...
    pub fn add_input(
        ...
        self.inputs.insert(
            ...
            VideoQueueInput {
                ...
                offset_handler: opts.offset.map(OffsetHandler::new),
    ...
    ///
    /// Additionally, following cases are considered ready:
    /// - Inputs with offset, where offset is in the future
    /// - Inputs without offset, before first chunk is received
...
pub struct VideoQueueInput {
    ...
    offset_handler: Option<OffsetHandler>,
...
impl VideoQueueInput {
    ...
    fn get_frame(
        ...
        let frame = match &self.offset_handler {
            // if stream has offset and should not start yet, do not send any frames
            Some(handler) if handler.before_start(buffer_pts, queue_start_pts) => None,
    ...
    fn drop_old_frames_before_start(&mut self) {
        if self.offset_handler.is_some() {
    ...
    fn is_required_ready_for_pts(
        ...
        match &self.offset_handler {
            // if offset is in the past then input is required for the next batch
            Some(offset) => !offset.before_start(next_buffer_pts, queue_start_pts),
            None => true,
        }
    }
                ...
                Some(queue_start_pts) => {
                    ...
                    match frame {
                        ...
                        PipelineEvent::Data(mut frame) => {
                            frame.pts = offset.calculate_pts(frame.pts, queue_start_pts);
