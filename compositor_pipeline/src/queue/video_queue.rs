...
use crate::{
    ...
    queue::{utils::EmitEventOnce, QueueVideoOutput},
};
...
impl VideoQueueInput {
        ...
        if self.offset.is_none() {
            match self.receiver.try_recv()? {
                PipelineEvent::Data(frame) => {
                    let _ = self.first_pts.get_or_insert(frame.pts);
            ...
            match self.receiver.try_recv()? {
                ...
                PipelineEvent::Data(mut frame) => {
                    let first_pts = *self.first_pts.get_or_insert(frame.pts);
    ...
    fn offset_pts(&self, queue_start_pts: Duration) -> Option<Duration> {
        self.offset.map(|offset| queue_start_pts + offset)
    }
}
