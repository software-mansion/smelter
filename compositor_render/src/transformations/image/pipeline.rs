...
#[derive(Debug)]
pub struct RemoveAlphaPremult {
    pipeline: wgpu::RenderPipeline,
    sampler: Sampler,
}

impl RemoveAlphaPremult {
    pub fn new(
        device: &wgpu::Device,
        rgba_textures_bind_group_layout: &wgpu::BindGroupLayout,
    ) -> Self {
        let shader_module =
            device.create_shader_module(wgpu::include_wgsl!("remove_alpha_premult.wgsl"));
    ...
    pub fn convert(&self, ctx: &WgpuCtx, src: (&RGBATexture, &wgpu::BindGroup), dst: &Texture) {
        let mut encoder = ctx
            .device
