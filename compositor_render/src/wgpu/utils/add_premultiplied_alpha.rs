...
#[derive(Debug)]
pub struct AddPremultipliedAlpha {
    pipeline: wgpu::RenderPipeline,
    sampler: Sampler,
}

impl AddPremultipliedAlpha {
    pub fn new(
        device: &wgpu::Device,
        ...
        let shader_module =
            device.create_shader_module(wgpu::include_wgsl!("add_premultiplied_alpha.wgsl"));
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("Add pre-multiplied alpha render pipeline"),
    ...
    pub fn render(&self, ctx: &WgpuCtx, src_bg: &wgpu::BindGroup, dst_view: &wgpu::TextureView) {
        let mut encoder = ctx
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("Add pre-multiplied alpha encoder"),
            });

        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("Add pre-multiplied alpha render pass"),
