...
use smelter_api::{
    DeckLink, HlsInput, HlsOutput, ImageSpec, InputId, Mp4Input, Mp4Output, OutputId,
    RendererId, RtmpInput, RtmpOutput, RtpInput, RtpOutput, ShaderSpec, V4l2Input,
...
pub async fn handle_input(
    ...
    tokio::task::spawn_blocking(move || {
        let response = match request {
            RegisterInput::RtpStream(rtp) => Pipeline::register_input(
                &api.pipeline()?,
                input_id.into(),
                rtp.try_into()?,
            )?,
            RegisterInput::RtmpServer(rtmp) => Pipeline::register_input(
                &api.pipeline()?,
                input_id.into(),
                rtmp.try_into()?,
            )?,
            RegisterInput::Mp4(mp4) => Pipeline::register_input(
                &api.pipeline()?,
                input_id.into(),
                mp4.try_into()?,
            )?,
            RegisterInput::DeckLink(decklink) => Pipeline::register_input(
                &api.pipeline()?,
                input_id.into(),
                decklink.try_into()?,
            )?,
            RegisterInput::WhipServer(whip) => Pipeline::register_input(
                &api.pipeline()?,
                input_id.into(),
                whip.try_into()?,
            )?,
            RegisterInput::WhepClient(whep) => Pipeline::register_input(
                &api.pipeline()?,
                input_id.into(),
                whep.try_into()?,
            )?,
            RegisterInput::Hls(hls) => Pipeline::register_input(
                &api.pipeline()?,
                input_id.into(),
                hls.try_into()?,
            )?,
            RegisterInput::V4l2(v4l2) => Pipeline::register_input(
                &api.pipeline()?,
                input_id.into(),
                v4l2.try_into()?,
            )?,
        ...
        match response {
            InputInitInfo::Rtp { port } => {
                Ok(Response::RegisteredPort { port: port.map(|p| p.0) })
            }
            ...
            InputInitInfo::Whip { bearer_token, endpoint_route } => {
                Ok(Response::RegisteredWhipInput { bearer_token, endpoint_route })
            }
...
pub async fn handle_output(
    ...
    tokio::task::spawn_blocking(move || {
        let response = match request {
            RegisterOutput::RtpStream(rtp) => Pipeline::register_output(
                &api.pipeline()?,
                output_id.into(),
                rtp.try_into()?,
            )?,
            RegisterOutput::Mp4(mp4) => Pipeline::register_output(
                &api.pipeline()?,
                output_id.into(),
                mp4.try_into()?,
            )?,
            RegisterOutput::WhipClient(whip) => Pipeline::register_output(
                &api.pipeline()?,
                output_id.into(),
                whip.try_into()?,
            )?,
            RegisterOutput::WhepServer(whep) => Pipeline::register_output(
                &api.pipeline()?,
                output_id.into(),
                whep.try_into()?,
            )?,
            RegisterOutput::RtmpClient(rtmp) => Pipeline::register_output(
                &api.pipeline()?,
                output_id.into(),
                rtmp.try_into()?,
            )?,
            RegisterOutput::Hls(hls) => Pipeline::register_output(
                &api.pipeline()?,
                output_id.into(),
                hls.try_into()?,
            )?,
...
pub async fn handle_font(
    ...
    let Some(field) =
        multipart.next_field().await.map_err(|err| ApiError::malformed_request(&err))?
    else {
        return Err(ApiError::malformed_request(&"Missing font file"));
    };

    let bytes = field.bytes().await.map_err(|err| ApiError::malformed_request(&err))?;

    let binary_font_source = Source::Binary(Arc::new(bytes));

    tokio::task::spawn_blocking(move || {
        api.pipeline()?.lock().unwrap().register_font(binary_font_source);
        Ok(Response::Ok {})
    })
    .await
    .unwrap()
