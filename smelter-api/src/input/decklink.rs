...
pub struct DeckLink {
    ...
    ///
    /// The input device is selected based on fields `subdevice_index`, `persistent_id` **AND**
    /// `display_name`. All of them need to match the device if they are specified. If nothing is
    /// matched, the error response will list available devices.
    ...
    /// Persistent ID of a device represented by 32-bit hex number. Each DeckLink sub-device has a
    /// separate id.
    ///
    /// The input device is selected based on fields `subdevice_index`, `persistent_id` **AND**
    /// `display_name`. All of them need to match the device if they are specified. If nothing is
