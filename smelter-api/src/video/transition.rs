...
pub struct Transition {
    ...
    /// On scene update, if there is already a transition in progress, it will be interrupted and
    /// the new transition will start from the current state.
    ///
    /// Defaults to `false`.
...
///
/// Custom easing functions can be implemented with cubic Bézier. The control points are defined
/// with `points` field by providing four numerical values: `x1`, `y1`, `x2` and `y2`. The `x1` and
/// `x2` values have to be in the range `[0; 1]`. The cubic Bézier result is clamped to the range
/// `[0; 1]`. You can find example control point configurations [here](https://easings.net/).
#[derive(Debug, Serialize, Deserialize, Clone, JsonSchema, ToSchema, PartialEq)]
