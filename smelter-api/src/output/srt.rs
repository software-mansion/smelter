...
///
/// Smelter binds an SRT listener on the configured `port` and accepts the
/// first caller that connects. While a caller is connected, further callers
/// are refused; when the active caller disconnects, the listener is re-opened
