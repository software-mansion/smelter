...
fn client_code() -> Result<()> {
    ...
    let path_loud = Path::new(PATH).join("examples/assets/pianoLoud.mp4");
    let path_quiet = Path::new(PATH).join("examples/assets/pianoQuiet.mp4");

    ffmpeg_audio_stream_from_file(&path_loud, IP, INPUT_PORT_1)?;
    ffmpeg_audio_stream_from_file(&path_loud, IP, INPUT_PORT_2)?;
    ffmpeg_audio_stream_from_file(&path_loud, IP, INPUT_PORT_3)?;
    ffmpeg_audio_stream_from_file(&path_quiet, IP, INPUT_PORT_4)?;
