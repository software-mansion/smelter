...
/// Standard RTMP publishing negotiation: connect -> createStream -> publish
/// (per RTMP spec sections 7.2.1.1, 7.2.1.3, 7.2.2.6).
/// Non-standard commands like `releaseStream` and `FCPublish` (sent by FFmpeg/OBS)
/// are handled outside the state machine in `handle_command_message`.
...
fn handle_command_message(
    ...
    match cmd {
        ...
        "connect" => {
            ...
            // this event is sent on stream ID 0 after the application connect command is successfully received from the client 7.1.7
        ...
        "publish" => {
            ...
            // Per spec section 7.1.7: StreamBegin signals the stream is now functional.
            // Sent before onStatus per the publish flow (spec section 7.2.2.6).
            ...
            trace!("Sent publish StreamBegin + onStatus response");
