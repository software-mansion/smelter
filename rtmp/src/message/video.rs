...
mod tests {
    ...
    #[test]
    fn serializes_enhanced_video_without_mod_ex_when_peer_does_not_support_it() {
        let raw = VideoMessage::Data(VideoData {
            track_id: TrackId::PRIMARY,
            codec: RtmpVideoCodec::Vp9,
            pts: Duration::from_nanos(100_000_777),
            dts: Duration::from_nanos(100_000_777),
            data: Bytes::from_static(b"vp9"),
            is_keyframe: false,
        })
        ...
        assert_eq!(raw.payload[0], 0xA1);

        match VideoMessage::from_raw(raw).unwrap() {
            VideoMessage::Data(data) => {
                assert_eq!(data.dts.as_nanos(), 100_000_000);
    ...
    #[test]
    fn serializes_enhanced_video_with_mod_ex_when_peer_supports_it() {
        let raw = VideoMessage::Data(VideoData {
            track_id: TrackId::PRIMARY,
            codec: RtmpVideoCodec::Vp9,
            pts: Duration::from_nanos(100_000_777),
