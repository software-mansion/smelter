    ...
    fn new(
        ...
        unsafe {
            device
                .device
                .video_queue_ext
                .cmd_end_video_coding_khr(**command_buffer, &vk::VideoEndCodingInfoKHR::default());
        }
...
impl VulkanEncoder<'_> {
    ...
    fn begin_video_coding(&self) {
        let layers = self.rate_control_layers_for(self.session_resources.rate_control);
        ...
        let reference_slot_info = self.session_resources.dpb.reference_slot_info();
    ...
    fn issue_coding_control_reset_for(&mut self, rate_control: RateControl) {
        ...
        let layers = self.rate_control_layers_for(rate_control);
        let mut h264_rate_control = self.h264_rate_control(layers.as_ref().map(|o| &o[..]));
    ...
    pub fn encode_bytes(
        ...
        if is_idr {
            ...
            self.issue_coding_control_reset_for(RateControl::Vbr {
        ...
        let primary_pic_type = if is_idr {
            vk::native::StdVideoH264PictureType_STD_VIDEO_H264_PICTURE_TYPE_I
        } else {
        ...
        println!("coding with {} references", reference_slots.len());
        ...
        println!(
            "fence triggered after {}s, feedback: {:?}",
    ...
    fn rate_control_layers_for(
        &self,
        rate_control: RateControl,
    ) -> Option<[vk::VideoEncodeRateControlLayerInfoKHR; 1]> {
