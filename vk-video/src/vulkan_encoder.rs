...
impl VulkanEncoder<'_> {
    ...
    pub fn new_with_converter(
        ...
        let enc = Self::new(device.clone(), profile, width, height, gop_size, rate_control)?;

        enc.command_buffers.encode_buffer.begin()?;

        let conv = Converter::new(
            device.clone(), width, height, &enc.profile_info, &enc.command_buffers.encode_buffer
        ).unwrap();

        enc.command_buffers.encode_buffer.end()?;
