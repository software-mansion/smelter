...
use crate::{
    ...
    wrappers::{
        ...
        Fence, Image, ImageView, ProfileInfo, QueryPool, Semaphore, TransferDirection,
...
impl EncodingQueryPool {
    ...
    pub(crate) fn get_result_blocking(&self) -> Result<EncodeFeedback, VulkanCtxError> {
        let mut result = EncodeFeedback { bytes_written: 0, status: vk::QueryResultStatusKHR::NOT_READY };
        unsafe {
            self.pool.device.get_query_pool_results(
                self.pool.pool,
                0,
                std::slice::from_mut(&mut result),
                vk::QueryResultFlags::WAIT | vk::QueryResultFlags::WITH_STATUS_KHR | vk::QueryResultFlags::TYPE_64,
            )?
        };
...
pub struct VulkanEncoder<'a> {
    ...
    active_reference_slots:
        std::collections::VecDeque<(usize, vk::native::StdVideoEncodeH264ReferenceInfo)>,
}
...
impl VulkanEncoder<'_> {
    ...
    pub fn encode_bytes(
        ...
        if is_idr {
            ...
            if self.active_reference_slots.len()
                == self
                    .device
                    .encode_capabilities
                    ...
                    .unwrap()
                    .video_capabilities
                    .max_active_reference_pictures as usize
                    + 1
            {
        ...
        if is_idr {
            ...
            let mut h264_rate_control = vk::VideoEncodeH264RateControlInfoKHR::default()
            ...
            let mut encode_rate_control = vk::VideoEncodeRateControlInfoKHR::default()
            ...
            let control_info = vk::VideoCodingControlInfoKHR::default()
                ...
                .push_next(&mut encode_rate_control)
                .push_next(&mut h264_rate_control);
        ...
        // TODO: this
        let idr_pic_id = if is_idr { self.idr_pic_id } else { 0 };
        ...
        let primary_pic_type = if is_idr { 0 } else { 2 };
        ...
        let slice_header = vk::native::StdVideoEncodeH264SliceHeader {
            ...
            first_mb_in_slice: 0,
            slice_type: if is_idr { 2 } else { 0 }, // TODO: b-frames
        ...
        let ref_lists = vk::native::StdVideoEncodeH264ReferenceListsInfo {
            ...
            num_ref_idx_l0_active_minus1: self.active_reference_slots.len() as u8,
        ...
        let setup_reference_slot = self.session_resources.dpb.allocate_reference_picture()?;
        ...
        let mut reference_slots = self
            ...
            .reference_slot_info()
            .into_iter()
            .filter(|i| i.slot_index >= 0 && i.slot_index != setup_reference_slot as i32)
            .collect::<Vec<_>>();

        let std_reference_info = reference_slots
            .iter()
            .flat_map(|s| {
                self.active_reference_slots
                    .iter()
                    .find(|(slot, _)| s.slot_index == *slot as i32)
                    .map(|(_, info)| *info)
            })
            .collect::<Vec<_>>();

        let mut reference_info = std_reference_info
            .iter()
            .map(|std| vk::VideoEncodeH264DpbSlotInfoKHR::default().std_reference_info(std))
            .collect::<Vec<_>>();

        reference_slots
            .iter_mut()
            .zip(reference_info.iter_mut())
            .for_each(|(slot, info)| *slot = slot.push_next(info));
        ...
        let setup_reference_slot_video_resource_info = self
            .session_resources
            .dpb
            .video_resource_info(setup_reference_slot)
            .unwrap();

        let setup_reference_slot = vk::VideoReferenceSlotInfoKHR::default()
            .slot_index(setup_reference_slot as i32)
        ...
        self.query_pool.begin_query(*self.command_buffers.encode_buffer);
        ...
        self.query_pool.end_query(*self.command_buffers.encode_buffer);
        ...
        println!("fence triggered after {}s, encoded {} bytes, status: {:?}", start.elapsed().as_secs_f64(), feedback.bytes_written, feedback.status);
