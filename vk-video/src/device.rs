...
use crate::{
    wrappers::*, BytesDecoder, DecoderError, H264Profile, RateControl, RawFrameData,
    VulkanEncoderError, VulkanInitError, VulkanInstance, WgpuTexturesDecoder,
};
...
impl VulkanDevice {
    ...
    pub fn crate_encoder(
        ...
        VulkanEncoder::new(self.clone(), parameters)
    }
    ...
    pub fn create_converter(
        ...
        let parameters = self.validate_and_fill_encoder_parameters(parameters)?;
        VulkanEncoder::new_with_converter(self.clone(), parameters)
    }
    ...
    fn validate_and_fill_encoder_parameters(
        ...
        if idr_period == 0 {
            return Err(VulkanEncoderError::ParametersError {
                field: "idr_period",
                problem: "IDR period cannot be 0.".into(),
            });
        }
        ...
        if width < min_extent.width || width > max_extent.width {
            return Err(VulkanEncoderError::ParametersError {
                field: "width",
                problem: format!(
        ...
        if height < min_extent.height || height > max_extent.height {
            return Err(VulkanEncoderError::ParametersError {
                field: "height",
                problem: format!(
        ...
        if framerate.0 == 0 || framerate.1 == 0 {
            return Err(VulkanEncoderError::ParametersError {
                field: "framerate",
                problem: format!(
                    "Framerate is {framerate:?}. Both numerator and denominator should be != 0.",
                ),
            });
        }
