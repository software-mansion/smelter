...
use crate::{wrappers::ImageView, VulkanCtxError};
...
impl Framebuffer {
    pub(crate) fn new(
        ...
        Ok(Self { framebuffer, attachments, render_pass, device })
    }
}
...
impl Sampler {
    pub(crate) fn new(device: Arc<Device>, create_info: &vk::SamplerCreateInfo) -> Result<Self, VulkanCtxError> {
        let sampler = unsafe {
...
impl DescriptorPool {
    pub(crate) fn new(device: Arc<Device>, create_info: &vk::DescriptorPoolCreateInfo) -> Result<Self, VulkanCtxError> {
        let pool = unsafe {
