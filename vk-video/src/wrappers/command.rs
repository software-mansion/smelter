...
use crate::{vulkan_ctx::Queue, VulkanCtxError, VulkanDevice};
...
pub(crate) struct CommandPool {
    ...
    pub(crate) queue_family_index: usize,
}
...
impl CommandPool {
    ...
    pub(crate) fn queue(&self) -> Result<&Queue, VulkanCtxError> {
        self.device.queue_from_index(self.queue_family_index)
    }
}
...
impl CommandBuffer {
    pub(crate) fn new_primary(pool: Arc<CommandPool>) -> Result<Self, VulkanCtxError> {
        ...
        let buffer = unsafe { pool.device.device.allocate_command_buffers(&allocate_info)?[0] };
