...
impl WgpuTexturesEncoder {
    ...
    /// # Safety
    ///
    /// - A [`wgpu::CommandBuffer`] with commands that render the [`wgpu::Texture`] in `frame` must
    ///   be submitted to the [`wgpu::Queue`] initialized by `vk_video`.
    /// - A `transition_resources` command with the folowing transition must be placed at the end
    ///   of this [`wgpu::CommandBuffer`]:
    /// ```no_run
    ...
    /// # ;
    /// ```
    /// - No commands must be submitted until this function returns.
