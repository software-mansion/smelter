...
use crate::{
    wrappers::{
        ...
        ImageView, Pipeline, PipelineLayout, RenderPass, Sampler, ShaderModule,
    },
...
impl Converter {
    ...
    pub(crate) fn convert(
    ...
    ) -> Result<(Arc<Mutex<Image>>, ImageView), YuvConverterError> {
        let image = unsafe {
        ...
        self.pipeline_y.convert(&self.command_buffer, &view);
        self.pipeline_uv.convert(&self.command_buffer, &view);
...
impl ConvertingPipeline {
    ...
    fn convert(&self, command_buffer: &CommandBuffer, view: &ImageView) {
