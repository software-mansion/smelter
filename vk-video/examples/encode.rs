...
fn main() {
    ...
    let mut encoder = vulkan_device
        .crate_encoder(vk_video::H264Profile::High, width, height)
        .expect("create encoder");

    let mut output_file = std::fs::File::create("output.h264").unwrap();

    let mut frame_buffer = vec![0; width as usize * height as usize * 3 / 2];
    while let Ok(()) = nv12.read_exact(&mut frame_buffer) {
        let frame = Frame {
            frame: RawFrame {
        ...
        let h264 = encoder.encode_bytes(frame).expect("encode");
