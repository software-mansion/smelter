#[cfg(vulkan)]
use vk_video::{ConverterState, RgbaNV12Converter};
    ...
    for i in 0..frame_count {
        ...
        let res = unsafe {
            encoder
                .encode(
                    Frame {
                        data: wgpu_state.converter_state.output_nv12().clone(),
                        pts: None,
                    },
                    false,
                )
...
impl WgpuState {
    fn new(
        ...
        let mut converter = RgbaNV12Converter::new(&device);
        let converter_state = converter.create_state(&view).unwrap();
    ...
    fn render(&self, time: f32) {
        ...
        self.converter.convert(&mut encoder, &self.converter_state);
