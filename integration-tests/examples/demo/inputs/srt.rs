...
impl From<SrtInputOptions> for SrtInput {
    fn from(value: SrtInputOptions) -> Self {
        let name = SrtInputBuilder::generate_input_name();
        Self {
            name,
            options: value,
        }
...
impl SrtInput {
    ...
    pub fn on_after_registration(&self) -> Result<()> {
        let input_path = match &self.options.path {
            ...
            None => TestSample::BigBuckBunnyH264AAC
                .file()
                .to_str()
                .unwrap()
                .to_string(),
        ...
        let srt_port = std::env::var("SMELTER_SRT_SERVER_PORT")
            .ok()
            .and_then(|s| s.parse::<u16>().ok())
            .unwrap_or(9710);
        let cmd = format!(
            "ffmpeg -re -i {input_path} {video_args}{audio_args}-f mpegts 'srt://127.0.0.1:{srt_port}?mode=caller&pkt_size=1316&streamid={stream_id}&passphrase=smelter1234&pbkeylen=16'",
            stream_id = self.name,
        );
...
impl SrtInputBuilder {
    pub fn new() -> Self {
        let name = Self::generate_input_name();
        Self {
            name,
            video: None,
            audio: None,
    ...
    pub(super) fn generate_input_name() -> String {
    ...
    fn prompt_path(self) -> Result<Self> {
        ...
        let default_path = TestSample::BigBuckBunnyH264AAC.file();
