"""Minimal sync side-channel subscriber.
...
from smelter import SideChannelInfo, SideChannelKind, list_channels
from smelter.sync import connect_audio, connect_video


def _consume_video(info: SideChannelInfo) -> None:
...
def _consume_audio(info: SideChannelInfo) -> None:
    tag = f"audio[{info.input_id}]"
    with connect_audio(info) as conn:
        for batch in conn:
            print(
...
def main() -> None:
    ...
    for info in channels:
        ...
        t = threading.Thread(target=target, args=(info,), daemon=True)
        t.start()
        threads.append(t)
